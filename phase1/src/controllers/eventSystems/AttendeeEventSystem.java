package controllers.eventSystems;

import gateways.IGateway;
import useCase.accountManager.AccountManager;
import useCase.eventManager.AttendeeEventManager;

import java.util.InputMismatchException;


/**
 * Event System for an attendee. User can perform operations related to events.
 */
public class AttendeeEventSystem extends EventSystem {
    private final AttendeeEventManager uem;
    private IGateway userGateway;

    /**
     * Constructs event system for an attendee.
     *
     * @param currUser     int id of the current user.
     * @param acm          AccountManager.
     * @param eventGateway EventGateway for read and write.
     * @param userGateway  UserGateway for read and write.
     */
    public AttendeeEventSystem(int currUser, AccountManager acm, IGateway eventGateway, IGateway userGateway) {
        super(currUser, acm, eventGateway);
        uem = new AttendeeEventManager(acm.getUserById(currUser));
        this.userGateway = userGateway;
    }

    /**
     * Takes user input of an event id to sign up the event.
     */
    public void signUpEvent() {
        boolean legalInput = false;
        getAllEvents();
        do {
            try {
                System.out.println("Please enter <event id> to sign up: ");
                int eventId = s.nextInt();
                s.nextLine();
                legalInput = true;
                boolean success = uem.signUpEvent(eventGateway, userGateway, eb, acm, eventId);
                if (success) {
                    System.out.println("You have signed up for event #" + eventId + "!");
                } else {
                    System.out.println("Sign Up failed!");
                }
            } catch (InputMismatchException e) {
                System.out.println("Illegal input!");
                s.next();
            }
        } while (!legalInput);
    }

    /**
     * Takes user input of an event id to cancel sign up.
     */
    public void cancelEvent() {
        boolean legalInput = false;
        getMySchedule();
        do {
            try {
                System.out.println("Please enter <event id> to cancel: ");
                int eventId = s.nextInt();
                s.nextLine();
                legalInput = true;
                boolean success = uem.cancelEvent(eventGateway, userGateway, eb, acm, eventId);
                if (success) {
                    System.out.println("You have cancelled event #" + eventId + "!");
                } else {
                    System.out.println("Cancelling failed!");
                }
            } catch (InputMismatchException e) {
                System.out.println("Illegal input!");
                s.next();
            }
        } while (!legalInput);
    }


    /**
     * Displays a schedule for the events the attendee has signed up for.
     */
    public void getMySchedule() {
        System.out.println("This is your schedule: ");
        presenter.presentSchedule(uem.getMySchedule(eb));
    }

    /**
     * Takes an command and make attendee event system operations.
     *
     * @param command an integer represents the operation.
     */
    @Override
    public void navigateCommand(int command) {
        switch (command) {
            case 0:
                break;
            case 1:
                getAllEvents();
                break;
            case 2:
                getAllSchedule();
                break;
            case 3:
                getEvent();
                break;
            case 4:
                signUpEvent();
                break;
            case 5:
                cancelEvent();
                break;
            case 6:
                getMySchedule();
                break;
        }
    }

    /**
     * Runs the event system of an attendee.
     *
     * @return 0 if the program execute successfully.
     */
    @Override
    public int run() {
        int choice = -1;
        do {
            try {
                System.out.println("----------------\nEvent System Menu: ");
                System.out.println("" +
                        "[1] Get all events\n" +
                        "[2] Get all schedule\n" +
                        "[3] Get information for specific event\n" +
                        "[4] Sign up for an event\n" +
                        "[5] Cancel my event\n" +
                        "[6] View my schedule\n" +
                        "[0] Exit");
                choice = s.nextInt();
                s.nextLine();
                if (choice >= 0 && choice <= 6) {
                    navigateCommand(choice);
                } else {
                    System.out.println("Illegal command!");
                }
            } catch (InputMismatchException e) {
                System.out.println("Illegal input!");
                s.next();
            }
        } while (choice != 0);
        return 0;
    }
}
